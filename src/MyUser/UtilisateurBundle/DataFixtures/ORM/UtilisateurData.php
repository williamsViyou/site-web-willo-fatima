...
use  MyUser\UtilisateurBundle\Entity\User;
...

{
...

    {
        ...
        $utilisateur5->setPanier(array());
        
        $manager->persist($utilisateur5);



        $utilisateur6 = new User();

        $utilisateur6->setUsername('toto');

        $utilisateur6->setEmail('toto@gmail.com');

        $utilisateur6->setEnabled(1);

        $utilisateur6->setPassword($this->container->get('security.encoder_factory')->getEncoder($utilisateur6)->encodePassword('toto', $utilisateur6->getSalt()));
     
        $utilisateur6->setPanier(array());
